...
def _put_resource_policy(data):
    ...
    policy = {
        ...
        # The scope every policy written through policyName carries on AWS. The
        # RESOURCE scope belongs to the resourceArn form, which is not modeled.
        "policyScope": "ACCOUNT",
    }
...
def _delete_resource_policy(data):
    name = data.get("policyName")
    if not name:
        # policyName is optional on the API model only because resourceArn is
        # the alternative addressing mode; without either there is nothing to
        # delete, and answering ResourceNotFoundException for "[None]" would
        # report a policy the caller never named.
        return error_response_json(
            "InvalidParameterException", "policyName is required.", 400,
        )
