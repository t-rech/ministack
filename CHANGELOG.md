...
### Added
- **CloudWatch Logs — resource policies** — `PutResourcePolicy`, `DescribeResourcePolicies` and `DeleteResourcePolicy` were unimplemented, so they answered `InvalidOperationException` ("Unknown action") and a `terraform apply` of any stack declaring `aws_cloudwatch_log_resource_policy` failed at the create — the resource used to grant a service permission to write to a log group, which is how EventBridge bus logging is authorized. All three are now served over the account/region-scoped store, and the policy is returned in AWS's `ResourcePolicy` shape (`policyName`, `policyDocument`, `lastUpdatedTime`, `policyScope: ACCOUNT`) so a `plan`/`refresh` reads back what it wrote. The AWS quota of **10 resource policies per account per region** is enforced — the 11th `PutResourcePolicy` fails with `LimitExceededException` while updating an existing policy at the limit still succeeds — because the limit is not adjustable on AWS, so a fleet design that spends one policy per resource has to hit the same wall locally to be worth testing against. `policyDocument` must be valid JSON and `policyName` is required, both rejected with `InvalidParameterException`. Policies are included in the service's persisted state, so a warm boot restores them and they keep counting against the quota. CloudFormation's `AWS::Logs::ResourcePolicy` now provisions through the same operation instead of minting a physical id and storing nothing, so a stack-declared policy is readable through `DescribeResourcePolicies`, counts against the same quota, and is removed when the stack is (a rename drops the policy under the old name rather than orphaning it). Not modeled: the `resourceArn` form and its `RESOURCE` policy scope, `revisionId`/`expectedRevisionId` optimistic concurrency, `DescribeResourcePolicies` pagination (the store is capped at ten), and enforcement — the store is read back, never consulted to authorize a write, matching MiniStack's permissive local IAM stance.
