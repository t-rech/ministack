...
- **EventBridge — archive/replay list pagination, documented list shapes, and terminal `CANCELLED`** — `ListArchives`/`ListReplays` now support `Limit` (1–100) + opaque `NextToken` (the `ListRules` pagination pattern) and return exactly the documented response members: `ListArchives` previously serialized the raw store record — **captured events included** — onto the wire, and `ListReplays` now carries `EventStartTime`/`EventEndTime`/`EventLastReplayedTime`/`ReplayEndTime`/`StateReason` (and drops `ReplayArn`, which the documented `Replay` shape does not have). `CANCELLED` is now terminal: the replay dispatch thread previously ended by stamping `COMPLETED` unconditionally, so a `CancelReplay` that won the race was silently overwritten — an illegal state transition; the thread now checks for cancellation before starting, per event, and before completing. Replays flipped to `FAILED` when persisted state is restored now carry a `StateReason`. **Not modeled, by design:** the `CANCELLING` intermediate state (local transitions are instantaneous); the ten-active-concurrent-replay quota (replays complete synchronously here, so an active-replay wall can never bind); retention-expiry sweeps and the `CREATING`/`UPDATING` archive states; 1-minute replay pacing (event-time ordering is kept); exact `SizeBytes` accounting (approximated from stored record size); and list-filter exclusivity ("Filter parameters are exclusive") — filters combine conjunctively here.
